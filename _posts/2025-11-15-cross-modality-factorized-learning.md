...
이 제약 조건들은 중간 특징 맵 단계에서 MINE과 같은 신경망 기반 상호 정보량 추정기나 대비 학습 손실 함수(InfoNCE Contrastive Loss)를 사용해 구현됩니다.

---

## 3. 교차 어텐션(Cross-Attention) 기반 특징 융합

분리된 특징들은 멀티헤드 **교차 어텐션(cross-attention)** 매커니즘을 통해 최종 융합됩니다:
...
이를 통해 네트워크는 공유 정보가 부족할 때에만 고유 특징에 동적으로 집중할 수 있게 되며, 일부 센서가 오작동하는 상황(예: 카메라 화면이 어두워져 열화상 센서의 고유 특징이 우선적으로 사용되는 경우)에서도 강건한 세그멘테이션 결과를 도출해 냅니다.
